...
final class SettingsViewModel {
    ...
    func saveOllamaKey() {
        ...
        guard !key.isEmpty else {
            ...
            saveMessage = "Ollama API key removed"
            return
        }
        do {
            try settingsRepo.saveOllamaAPIKey(key)
            saveMessage = "Ollama API key saved ✓"
    ...
    func testLocalConnection() async {
        ...
        if !isReachable {
            ...
            connectionTestResult = .failure("Could not reach server. Check the URL and that Ollama is running.")
            return
        }
